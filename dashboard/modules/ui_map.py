...
def render():
    ...
    with col_map_4:
        map_categoria = st.selectbox(
            ...
            options=df_categorias['categoria_delito'].tolist() if not df_categorias.empty else ["Cargando..."]
        )
    ...
    if not df_clusters.empty and map_alcaldia and map_categoria and model_xgb and model_kmeans:
                ...
                try:
                    ...
                    if prob_violento >= 0.65: 
                        hotspots.append({
                            ...
                            'radius': 200 + (prob_violento * 800), # Radio dinámico
                            'color_rgb': get_color_from_probability(prob_violento)
                        })
                except Exception as e:
                    # st.error(f"Error en predicción para un cluster: {e}") # Descomentar para debug
                    pass 
