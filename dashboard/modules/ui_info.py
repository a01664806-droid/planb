...
def _safe_uniques(series: pd.Series) -> list:
    try:
        vals = sorted([v for v in series.dropna().unique().tolist() if v != ""])
    except Exception:
        vals = []
    return vals
...
def compute_eda_aggregates(df: pd.DataFrame):
    ...
    if {"anio_inicio","crime_classification"}.issubset(df.columns):
        ...
        year_class["anio_inicio"] = pd.Categorical(year_class["anio_inicio"], categories=range(2015, 2026), ordered=True)
    else:
    ...
    if {"anio_inicio","violence_type"}.issubset(df.columns):
        ...
        yearly_viol["anio_inicio"] = pd.Categorical(yearly_viol["anio_inicio"], categories=range(2015, 2026), ordered=True)
    else:
...
def render():
    ...
    # ⬇️ Carga TODO (sin límite por defecto)
    ...
    with st.sidebar:
        ...
        year_range = st.slider("Años", min_value=y_min, max_value=y_max, value=(max(y_min, 2016), y_max), step=1)
        ...
        if dels:
            ...
            delito_default = []
        delito_sel = st.multiselect("Delitos (Top 30 por defecto)", options=dels, default=delito_default)
    ...
    # Filtramos los delitos de bajo impacto
    df = df[df["delito_std"] != "Bajo Impacto"]
    ...
    if not df_metrics.empty:
        ...
        try:
            ...
            m1, m2 = st.columns(2)
            m1.metric("Crímenes Violentos de Noche (19:00-07:00)", f"{noche_pct:.1f}%")
            m2.metric("Crímenes Violentos de Día (07:00-19:00)", f"{dia_pct:.1f}%")
        except IndexError:
    ...
    with cU1:
        st.altair_chart(
            bar_chart(agg["year_counts"], "year", "count",
                      sort=list(range(2015, 2026)), title="Crímenes por Año",
                      color=THEME_PALETTE[0],
                      x_axis=_axis("Año"), y_axis=_axis("Número de casos")),
            use_container_width=True
        )
    with cU2:
        st.altair_chart(
            bar_chart(agg["month_counts"], "month_name", "count",
                      sort=[MONTH_NAMES[m] for m in range(1, 13)], title="Crímenes por Mes",
                      color=THEME_PALETTE[2],
                      x_axis=_axis("Mes"), y_axis=_axis("Número de casos")),
            use_container_width=True
        )
    st.altair_chart(
        bar_chart(agg["hour_counts"], "hour", "count", sort=list(range(24)),
                  ...
                  x_axis=_axis("Hora del día"), y_axis=_axis("Número de casos"), width=980),
        use_container_width=True
    )
    ...
    with cS1:
        st.altair_chart(
            barh_chart(agg["alc_counts"], "alcaldia", "count",
                       ...
                       x_axis=_axis("Número de casos"), y_axis=_axis("Alcaldía", orient="left")),
            use_container_width=True
        )
    with cS2:
        st.altair_chart(
            barh_chart(agg["col_counts"], "colonia", "count",
                       ...
                       x_axis=_axis("Número de casos"), y_axis=_axis("Colonia", orient="left")),
            use_container_width=True
        )
    st.altair_chart(
        barh_chart(agg["del_counts"], "delito", "count",
                   title="Top 10 Delitos por Frecuencia", color=THEME_PALETTE[5],
                   x_axis=_axis("Número de casos"), y_axis=_axis("Tipo de delito", orient="left"), height=320),
        use_container_width=True
    )
        ...
        if not v_counts.empty:
            ...
            st.altair_chart(
                donut_chart(v_counts, "violence", "count",
                            title="Violento vs No Violento", colors=colors),
                use_container_width=True
            )
        else:
    ...
    if not stacked.empty:
        ...
        ch = alt.Chart(stacked, **_cfg(width=900, height=420)).mark_bar().encode(
            ...
            y=alt.Y("alcaldia_std:N", sort=order_mun, axis=_axis("Alcaldía", orient="left")),
